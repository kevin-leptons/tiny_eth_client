...
class Diary {
    ...
    async getLogs(filter) {
        ...
        let safeBlockNumber =
            node.latestBlockNumer - this._reorganisationBlocks

        if (safeBlockNumber < 0 || (filter.toBlock > safeBlockNumber)) {
            throw new DiaryError('call to unsafe block number')
        }
        ...
        for (;;) {
            ...
            nowFilter.fromBlock = Math.max(
                nowFilter.toBlock,
                logs[logs.length - 1].blockNumber + 1
            )
        }
